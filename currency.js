...
function input_parser(input) {
	...
	//cycle through matches and get the output array/map as required.
	for (var i=0; i<amounts.length; i++) {
		//console.log('amount '+i + 'is ' + amounts[i]);
		info=basic_reg.exec(amounts[i].trim())
		//regex results:
		//console.log('info is :' + info);
		output[info[2]]=info[1];
		// for output object: key is dirty currency name, value is amount.
	}
	return output;
}
...
function runner (string,base) {
	console.log(string);
	//var test_string = '125.12321 rupees + 21.123 aussie dollars + 413 usd';
	//test_string = '123 usd';
	a=currency_code_mapper(input_parser(string))
	return sum_to_given_currency(base,a);
}	
			...
			$.ajax({
			  ...
			  beforeSend: function( xhr ) {
			    xhr.overrideMimeType( "application/json;" );
					...
					var final = runner(string, fx.base);
					$('.converted-amount').text(final);
			});
        }, doneTypingInterval);
    }
});
...
$(document).ready(function () {
	for (var country in codes) {
		if(codes.hasOwnProperty(country)) {
			console.log(country);
			$('#outputCurrency').append('<option value="' + country + '">' + codes[country].name + '</option>');
		}
	}
})
